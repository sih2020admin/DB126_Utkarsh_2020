...
router.post('/get_application', function (req, res) {
	...
	db_1.default.query('SELECT et.et_id, et.et_title, et.et_tender_fee, et.is_approved as approved,et.et_tender_ref_no, et.et_tender_desc, et.et_file_uri, et.et_bidding_date, v.vd_id,vc.vcd_id, vc.vcd_name, vc.vcd_dob, vc.vcd_aadhar, vc.vcd_contact, vc.vcd_email, vc.vcd_designation, v.v_name, v.v_address, v.v_yoe, v.v_email, v.v_mobile, v.v_reg_no, v.v_legal_id, v.v_pan, v.v_gst, v.schemes, v.staff, v.equipment, v.india_equipment, v.rating ,f.etd_id,cast( AES_DECRYPT(f.furi1 ,"'+key+'") as char ) as furi1, cast( AES_DECRYPT(f.furi2 ,"'+key+'") as char ) as furi2, p.txn_id, p.txn_amount, p.txn_timestamp, p.bank_name, p.resp_message, cast( AES_DECRYPT(e.bidding_amt ,"'+key+'") as char ) as bidding_amt, cast( AES_DECRYPT(e.location ,"'+key+'") as char ) as location, cast( AES_DECRYPT(e.is_approved ,"'+key+'") as char ) as is_approved, e.timestamp,cast( AES_DECRYPT(e.time_period ,"'+key+'") as char ) as time_period,e.equi FROM `e_tender_vendor` as e INNER JOIN e_tender_details as et ON et.et_id=e.et_id INNER JOIN `vendor_details` as v ON e.vd_id=v.vd_id INNER JOIN v_contact_details as vc ON e.vcd_id = vc.vcd_id INNER JOIN `file_uri` as f ON e.etd_id=f.etd_id INNER JOIN payment_transactions as p ON p.etd_id = e.etd_id WHERE et.et_id = ' + et_id + ';', function (error, results, fields) {
		if (error) {
			console.log("error", error);
			res.sendStatus(400);
			console.log("gettenderlist called0")
