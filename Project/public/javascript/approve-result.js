...
function app(j){
    ...
    document.getElementById("file1").innerHTML = res[j].furi1;
    a= document.getElementById("preview");
    a.href='https://165.22.210.37:8081/get_files?furi='+ res[j].furi1 + '&vd_id=' + res[j].vd_id + '&vcd_id=' + res[j].vcd_id;
    document.getElementById("file2").innerHTML = response[j].furi2;
    b= document.getElementById("preview1");
    b.href='https://165.22.210.37:8081/get_files?furi='+ res[j].furi2 + '&vd_id=' + res[j].vd_id + '&vcd_id=' + res[j].vcd_id;
}
    ...
    if(i==0){
        Swal.fire({
            ...
            if (results['value']) {
                    ...
                    xhr.send(JSON.stringify({ etd_id: etd_id, status: "-3",reason:results['value']}))
                    xhr.onload = function () {
                        if (this.status == 200) {
                            modal.style.display = "none";
        ...
        xhr.open('POST', url)
        xhr.setRequestHeader('Content-Type', 'application/json')
        xhr.send(JSON.stringify({ etd_id: etd_id, status: "3",reason:""}))
        xhr.onload = function () {
            if (this.status == 200) {
                modal.style.display = "none";
    ...
    if (this.status == 200) {
        ...
        if(response[0].approved==3){
            ...
            for (i = 0; i < response.length; i++) { 
                if(response[i].is_approved==2 || response[i].is_approved==3 || response[i].is_approved==-3){
                    res[k]=response[i];
                    k++;
                }
            }
            ...
            if (res.length > 0) {
                ...
                technical_div.insertAdjacentHTML('beforeend', `<br><br><p>NO APPLICATIONS FOR RESULT STAGE</p>`)
            }
            if(flag==1){
...
function display(){
    ...
    xhr.onload = function () {
        if (this.status == 200) {
            response = JSON.parse(this.responseText);
            var technical_div = document.getElementById('result');
            var flag=0,k=0;
            res=[];
            for (i = 0; i < response.length; i++) { 
            ...
            if (res.length > 0) {
                ...
                technical_div.insertAdjacentHTML('beforeend', `<br><br><p>NO BID WINNERS</p>`)
            }
            if(flag==1){
                technical_content  = technical_content+ `</table></div>`
                technical_div.insertAdjacentHTML('beforeend',technical_content);
