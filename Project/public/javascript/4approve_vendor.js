    ...
    success: function (response) {
        ...
        try {
            for (var response_1 = __values(response), response_1_1 = response_1.next(); !response_1_1.done; response_1_1 = response_1.next()) {
                ...
                "<label for='f1" + count + "'>Technical File</label><p id='f1" + count + "'>  <a target=`__blank` href='http://165.22.210.37:8085/get_files?furi=" + x.furi1 + "&vd_id="+x.vd_id+"&vcd_id="+x.vcd_id+"'>file_url</a></p> <p id='tech_exist" + count + "'> " + x.tech_uri + "</p> </p> <br>"+
                "<label for='f2" + count + "'>BOQ File</label><p id='f1" + count + "'> <a target=`__blank` href='http://165.22.210.37:8085/get_files?furi="  + x.furi2 + "&vd_id="+x.vd_id+"&vcd_id="+x.vcd_id+"'>file_url</a></p><p id='boq_exist" + count + "'> " + x.boq_uri + "</p> <br><br>"+
...
function approve(value) {
    alertify
        ...
        $.ajax({
            url: "/approve_tender_application",
            async: true,
            method: 'POST',
            data: {
