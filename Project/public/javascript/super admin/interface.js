$.post('/super-admin/approve-vendor').then((result) => {
    if (result.length > 0) {
        for (let i = 0; i < result.length; i++) {
            $('<div></div>', {
                ...
                html: ` <h2>Company Details</h2><br>
                    ...
                    <td class="fields">File URL</td><td><a target="_blank" href="/get_files_super?furi=${result[i]['furi']}&vcd_id=${result[i]['vcd_id']}">Link</a></td>
                    </tr>
                    </table><br><br>
                    <div class="butts">
        ...
        $('<div></div>', {
            class:"cont",
            html:`<h2>No Vendor Applications available for Approval</h2>`
        }).appendTo('#container')   
    }
})
$('#container').click((e) => {
