...
async function getProfileDetails(request) {
    ...
    let profile = await connection_1.default.query(` SELECT v_name, v_address, v_yoe, v_email, v_mobile, v_reg_no, v_state_id, v_city_id, v_pincode, v_legal_id, v_pan, v_is_verified, v_gst , vcd_id FROM vendor_details WHERE vd_id = '${vd_id}';
                                                ...
                                                SELECT e_tender_vendor.etd_id,e_tender_vendor.et_id ,et_title, et_tender_fee, et_tender_ref_no, et_tender_desc, et_last_date_apply, et_bidding_date, et_file_uri, dept_id,cast( AES_DECRYPT(e_tender_vendor.bidding_amt ,'${KEY}') as char ) as bidding_amt ,cast( AES_DECRYPT(e_tender_vendor.status ,'${KEY}') as char ) as status FROM e_tender_details INNER JOIN e_tender_vendor ON e_tender_details.et_id = e_tender_vendor.et_id WHERE e_tender_vendor.vd_id = '${vd_id}' and e_tender_vendor.vcd_id = '${vcd_id}';
...
async function confirmedTenderDetails(request) {
    let temp = await connection_1.default.query(`SELECT et_id,et_title,et_tender_fee,et_tender_ref_no,et_bidding_date FROM  e_tender_details WHERE et_id = '${request.query['et_id']}';
                                            SELECT * FROM (SELECT vendor_details.vd_id,vcd_id,vcd_name ,vcd_dob ,vcd_aadhar,vcd_contact,vcd_email,vcd_designation,v_name,v_address,v_yoe,v_email,v_mobile,v_reg_no,v_legal_id,v_pan,v_gst FROM v_contact_details,vendor_details WHERE v_contact_details.vd_id=vendor_details.vd_id) AS hello WHERE vd_id= '${request.signedCookies['vd_id_e']}';
                                            SELECT * FROM (SELECT file_uri.etd_id, cast( AES_DECRYPT(file_uri.furi1 ,"${KEY}") as char ) as furi1 ,cast( AES_DECRYPT(file_uri.furi2 ,"${KEY}") as char ) as furi2,txn_id,txn_amount,txn_timestamp,bank_name ,resp_message , etv.vcd_id FROM file_uri,payment_transactions , e_tender_vendor as etv WHERE file_uri.etd_id=payment_transactions.etd_id and file_uri.etd_id=etv.etd_id) AS hello WHERE etd_id= '${request.query['etd_id']}';
